...
def process_results(output=False):
    ...
    for d in data:
        ...
        for s in SCHEMES:
            ...
            pir_tputs_std[f"{s}_tput_std"] = np.std(d[f'server_{s}_processing_ms'])
        
        db_size_log2 = int(math.log2(d["db_size"]))
        elem_size_bits = 8*d["item_size"]
        
        if (db_size_log2, elem_size_bits) not in pirac_tput_results:
            pirac_tput_results[(db_size_log2, elem_size_bits)] = cal_pirac_tput(18, 
        ...
        row["xor_pirac_tput_std_pct"] = (100*row["xor_pirac_tput"]*
                                (pirac_tput_std/np.square(pirac_tput)+pir_tputs_std["xor_tput_std_pct"]/np.square(row["xor_tput"])))
        row["pir_pirac_tput_std_pct"] = (100*row["pir_pirac_tput"]*
                                (pirac_tput_std/np.square(pirac_tput)+pir_tputs_std["pir_tput_std_pct"]/np.square(row["pir_tput"])))
