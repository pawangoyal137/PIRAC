...
def process_results(output=False):
    ...
    for d in data:
        ...
        if (db_size_log2, item_size_bits) not in pirac_tput_results:
            pirac_tput_results[(db_size_log2, item_size_bits)] = benchmark_pirac([db_size_log2], 
                                                                    [item_size_bits], 10, rekeying = True)
        tput_pirac = pirac_tput_results[(db_size_log2, item_size_bits)]
        row["xor_pirac_tput"] = utils.cal_tput_with_pirac( [row["xor_tput"]], tput_pirac)[0]
        row["pir_pirac_tput"] = utils.cal_tput_with_pirac( [row["pir_tput"]], tput_pirac)[0]
